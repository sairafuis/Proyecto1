...
#Clase de nodo.
class Nodo:
	def __init__(self,idNodo,posicion = 0):
		self.idNodo = idNodo
...
class Grafo(Nodo,Vertice):
	...
	def crearNode(self,id):
		posicionx = round(random.uniform(600, 1100))
		posiciony = round(random.uniform(200, 700))
		posicion = (posicionx,posiciony)
		nodos = Nodo(id,posicion)
		return nodos
...
#Metodo para calcular cantidad de nodos
	...
	def nodosPosicion(self,nodos):
		nodosnew = []
		for i in range(0,nodos):
			nodosnew.append(Grafo(False).crearNode(i))
		return nodosnew
